...
program
    ...
    try {
        const [episodic, semantic, identity, embeddings, queue] = await Promise.all([
            pool.query('SELECT COUNT(*) FROM episodic_memories'),
            pool.query('SELECT COUNT(*) FROM semantic_nodes'),
            pool.query('SELECT COUNT(*) FROM identity_facts'),
            pool.query('SELECT COUNT(*) FROM embeddings'),
            pool.query(`SELECT COUNT(*) FROM write_queue WHERE status = 'pending'`),
        ]);
        console.log('ok: true');
...
program
    ...
    .action(async () => {
    try {
        const { rows } = await reader.query(`SELECT a.name, a.tier,
...
vpn
    ...
    try {
        ...
        const vpnSetup = await import(resolve(__dirname, '../../proxy/dist/vpn-setup.js'));
        const { installVPN } = vpnSetup;
        await installVPN();
    }
    catch (err) {
...
vpn
    ...
    try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const { uninstallVPN } = (await import(resolve(__dirname, '../../proxy/dist/vpn-setup.js')));
        await uninstallVPN();
    }
    catch (err) {
...
program
    ...
    .action(async (name) => {
    try {
        const { rows } = await reader.query('SELECT id FROM agents WHERE name = $1', [name]);
        if (!rows[0]) {
...
program
    ...
    try {
        const { rows } = await reader.query('SELECT COUNT(*) FROM profiles');
    ...
    try {
        const [e, s, i] = await Promise.all([
            reader.query('SELECT COUNT(*) FROM episodic_memories'),
            reader.query('SELECT COUNT(*) FROM semantic_nodes'),
            reader.query('SELECT COUNT(*) FROM identity_facts'),
        ]);
    ...
    try {
        const { rows: existing } = await reader.query(`SELECT t.token_hint FROM agent_tokens t JOIN agents a ON a.id = t.agent_id WHERE a.name = 'claude-code'`);
...
// SCI-212: passthrough unknown commands to the agent binary so `sci up`,
// `sci --setup`, `sci doctor` etc. work when installed via homebrew.
program.addHelpCommand(false);
program.allowUnknownOption(true);
program.action(async () => {
    const { spawnSync } = await import('child_process');
    const AGENT = resolve(__dirname, '../../agent/dist/index.js');
    const result = spawnSync(process.execPath, [AGENT, ...process.argv.slice(2)], { stdio: 'inherit' });
