...
function tokenInfoFromResponse(tok, prev) {
    return {
...
export async function login(opts = {}) {
    ...
    const code = await new Promise((resolve, reject) => {
        server.on('request', (req, res) => {
            ...
            let problem = null;
            if (error)
                problem = `oauth error: ${error}`;
            else if (!code)
                problem = 'callback missing code';
            else if (returnedState !== state)
                problem = 'state mismatch — possible CSRF (did you click an old auth URL?)';
        ...
        ${problem ? '<p style="color:#666;font-size:13px;">Re-run the auth command to start a fresh flow with a new state token.</p>' : ''}
            ...
            res.end(html);
            if (problem)
                return reject(new Error(problem));
            resolve(code);
        });
        setTimeout(() => reject(new Error(`timeout waiting for browser auth (${BROWSER_AUTH_TIMEOUT_MS / 1000}s)`)), BROWSER_AUTH_TIMEOUT_MS);
...
export async function refresh() {
    ...
    if (!cache?.refresh_token)
        throw new Error('no refresh_token cached — run login() first');
...
export async function getAccessToken() {
    const cache = readCache();
    if (!cache)
        throw new Error('no OAuth token cached — run `sci-ui-auth login` first');
