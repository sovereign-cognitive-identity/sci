...
const _hostToIp = new Map<string, string>()
const _ipToHost = new Map<string, string>()
...
export function assignFakeIP(hostname: string): string {
...
export function lookupByFakeIP(ip: string): string | undefined {
  return _ipToHost.get(ip)
}

/** Returns the fake IP for a hostname, or undefined if not registered. */
export function lookupFakeIP(hostname: string): string | undefined {
  return _hostToIp.get(hostname)
}

/** Returns true if the hostname is a known AI endpoint. */
export function isAIHostname(hostname: string): boolean {
  return AI_HOSTNAMES.includes(hostname as AIHostname)
}
...
export function warmFakeIPs(): void {
...
export function listFakeIPs(): Array<{ hostname: string; ip: string }> {
