...
export function parseSNI(buf) {
    try {
        if (buf.length < 5)
            return null;
        if (buf[0] !== 0x16)
            return null; // not TLS handshake
        if (buf.length < 5 + 4)
            return null; // need handshake header
        let p = 5; // skip record header
        if (buf[p] !== 0x01)
            return null; // not ClientHello
 ...
 */
export function peekClientHello(socket, timeoutMs = 3000) {
...
//# sourceMappingURL=sni-sniff.js.map