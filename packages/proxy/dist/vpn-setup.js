...
import { writeResolverEntries, removeResolverEntries, removeFakeIPRoute, stopSingBox, TUN_ADDR, RESOLVER_DOMAINS, } from './tun.js';
import { FAKE_IP_CIDR, AI_HOSTNAMES } from './fake-ip.js';
import { SOCKS5_PORT } from './socks5.js';
import { installCleanupHelper, RECOVER_SCRIPT } from './tun-guard.js';
// ── Install ───────────────────────────────────────────────────────────────────
export async function installVPN() {
    ...
    try {
        execSync(`sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain ${certPath}`, { stdio: 'pipe' });
        console.log('✓');
    }
    catch {
...
function findSingBox() {
    ...
    try {
        return execSync('which sing-box', { encoding: 'utf-8' }).trim();
    }
    catch {
        return null;
    }
...
// ── Uninstall ─────────────────────────────────────────────────────────────────
export async function uninstallVPN() {
    ...
    try {
        execSync('sudo security delete-certificate -c "Sci Local CA" /Library/Keychains/System.keychain 2>/dev/null || true', { stdio: 'pipe' });
        console.log('✓');
    }
    catch {
...
//# sourceMappingURL=vpn-setup.js.map