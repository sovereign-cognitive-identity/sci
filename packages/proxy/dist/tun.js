...
import { runRecoveryIfStale, writeRecoveryScript, registerTUNCleanup, } from './tun-guard.js';
...
export function startSingBox() {
    ...
    if (!singboxBin) {
        throw new Error('sing-box not found. Install it: brew install sing-box');
    }
    // Write config
    ...
    _proc.stdout?.on('data', (d) => process.stderr.write(`[singbox] ${d}`));
    _proc.stderr?.on('data', (d) => process.stderr.write(`[singbox] ${d}`));
 ...
 * Requires sudo.
 */
export function addFakeIPRoute() {
    try {
...
export function removeFakeIPRoute() {
    try {
        execSync(`sudo /sbin/route delete -net ${FAKE_IP_CIDR} 2>/dev/null || true`, { stdio: 'pipe' });
 ...
 */
export function addRealIPRoute(realIP) {
...
export function removeRealIPRoute(realIP) {
    const parts = realIP.split('.').map(Number);
    if (parts.length !== 4 || parts.some(isNaN))
        return;
    const cidr16 = `${parts[0]}.${parts[1]}.0.0/16`;
...
const _addedRealIPRoutes = new Set();
export function trackRealIPRoute(realIP) { _addedRealIPRoutes.add(realIP); }
export function listRealIPRoutes() { return [..._addedRealIPRoutes]; }
/** Wait for utun9 to appear (sing-box creates it asynchronously). */
export async function waitForInterface(timeoutMs = 5000) {
...
port ${DNS_PORT}
`;
export function writeResolverEntries() {
...
export function removeResolverEntries() {
    for (const domain of RESOLVER_DOMAINS) {
        try {
 ...
 *   7. Flush DNS
 */
export async function startTUNWithGuard(adapter) {
    ...
    registerTUNCleanup(async () => {
        stopSingBox();
        removeFakeIPRoute();
        for (const ip of listRealIPRoutes())
            removeRealIPRoute(ip);
        removeHostsEntries();
    ...
    try {
        ...
        const ANTHROPIC_ONLY = ['api.anthropic.com'];
