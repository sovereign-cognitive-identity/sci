...
function buildAResponse(query, txid, hostname, ip) {
    ...
    let off = 0;
    // Header
    response.writeUInt16BE(txid, off);
    off += 2; // Transaction ID
    response.writeUInt16BE(0x8180, off);
    off += 2; // Flags: QR=1, AA=1, RD=1, RA=1
    response.writeUInt16BE(1, off);
    off += 2; // QDCOUNT = 1
    response.writeUInt16BE(1, off);
    off += 2; // ANCOUNT = 1
    response.writeUInt16BE(0, off);
    off += 2; // NSCOUNT = 0
    response.writeUInt16BE(0, off);
    off += 2; // ARCOUNT = 0
    ...
    // Answer section
    response.writeUInt16BE(0xc00c, off);
    off += 2; // Name: pointer to offset 12
    response.writeUInt16BE(1, off);
    off += 2; // TYPE = A
    response.writeUInt16BE(1, off);
    off += 2; // CLASS = IN
    response.writeUInt32BE(300, off);
    off += 4; // TTL = 300s
    response.writeUInt16BE(4, off);
    off += 2; // RDLENGTH = 4
    for (const octet of ipParts) {
        response.writeUInt8(octet, off);
        off++;
    }
    return response.slice(0, off);
}
/** Build a DNS NOERROR response with no answers (suppresses AAAA without error). */
function buildEmptyResponse(txid, query) {
    const { nextOffset: questionEnd } = parseQueryName(query, 12);
    ...
    off += 2;
    response.writeUInt16BE(0x8180, off);
    off += 2; // QR=1, AA=1, RD=1, RA=1, RCODE=0
    response.writeUInt16BE(1, off);
    off += 2; // QDCOUNT=1
    response.writeUInt16BE(0, off);
    off += 2; // ANCOUNT=0
...
function forwardToRealDNS(query, txid, rinfo, server) {
