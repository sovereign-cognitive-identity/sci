...
export function startHTTPSServer(adapter, openrouterKey) {
    const ca = ensureCACert();
    // Minimal Hono-compatible context adapter for our handlers
    const makeContext = (req, body, hostname) => ({
        req: {
    ...
    const server = https.createServer({ SNICallback: makeSNICallback(ca) }, async (req, res) => {
 ...
 */
function forwardPipe(req, hostname, realIP, res) {
 ...
 */
function forwardToUpstream(req, body, hostname, realIP, res) {
...
async function pipeResponse(result, res) {
    const headers = {};
...
//# sourceMappingURL=server-https.js.map