 ...
 */
export async function injectMemoryContext(messages, adapter, sessionTokenMap) {
    ...
    try {
        ...
        const recallTypes = ['semantic', 'identity', 'episodic'];
        ...
        for (const r of rawResults) {
            ...
            if (r.type === 'episodic') {
                const role = r.metadata?.['role'] ?? null;
                if (role && role !== 'user')
                    continue;
            }
            ...
            seen.add(key);
            filtered.push(r);
            if (filtered.length >= MAX_CONTEXT_RESULTS)
                break;
        }
...
export async function storeInteraction(userMessage, assistantResponse, adapter) {
    try {
        const profile = await adapter.getProfile('work');
        if (!profile)
            return;
