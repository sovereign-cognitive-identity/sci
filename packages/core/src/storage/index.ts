...
export type { StorageAdapter, Profile, EpisodicMemory, SemanticNode, IdentityFact, RecallResult, StorageStats } from './interface.js'
  ...
  switch (backend) {
    case 'local': {
      ...
      const adapter = new LocalAdapter(
        process.env['SCI_DB_READER_URL'] ?? requireEnv('SCI_DB_READER_URL'),
        process.env['SCI_DB_WRITER_URL'] ?? requireEnv('SCI_DB_WRITER_URL'),
      )
...
function requireEnv(name: string): string {
  ...
  if (!val) throw new Error(`Missing required env var for storage backend: ${name}`)
  return val
}
