...
export class iCloudAdapter extends CloudAdapter {
  ...
  protected async _downloadIfNeeded(): Promise<void> {
    ...
    if (!existsSync(remoteDb)) return  // Nothing in iCloud yet
    ...
    if (!localExists) {
      ...
      if (existsSync(remoteIdx)) copyFileSync(remoteIdx, this.idxPath)
    }
  ...
  protected async _sync(): Promise<{ uploaded: number; downloaded: number }> {
    try {
      if (!existsSync(this.icloudDir)) mkdirSync(this.icloudDir, { recursive: true })
