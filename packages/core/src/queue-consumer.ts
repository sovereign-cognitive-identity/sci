  ...
  try {
    ...
    const { rows } = await client.query<QueueItem>(
      `UPDATE write_queue
       ...
       RETURNING id, operation, payload, status`,
      [BATCH_SIZE]
    )
    ...
    for (const item of rows) {
      try {
        await dispatch(item)
        await client.query(
          `UPDATE write_queue SET status = 'done', processed_at = NOW() WHERE id = $1`,
          [item.id]
        )
        processed++
      } catch (err) {
        ...
        const retries = ((item.payload.__retries as number) ?? 0) + 1
        if (retries >= MAX_RETRIES) {
          await client.query(
            `UPDATE write_queue SET status = 'failed', processed_at = NOW() WHERE id = $1`,
            [item.id]
          )
        } else {
          await client.query(
            `UPDATE write_queue
             ...
             WHERE id = $1`,
            [item.id, JSON.stringify({ __retries: retries })]
          )
        }
      }
    }
...
async function run() {
  ...
  const poll = async () => {
    try {
      ...
      if (n > 0) console.log(`[queue] Processed ${n} items`)
    } catch (err) {
