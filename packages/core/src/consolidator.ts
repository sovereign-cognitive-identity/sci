...
async function llmCall(system: string, user: string, model: string, apiKey: string): Promise<string> {
...
interface LLMFact { content: string; category: string; confidence: number }

export interface PromotionResult { promoted: number; reinforced: number; batches: number }

export async function runPromotionPass(
  windowStart: Date, windowEnd: Date,
  profileId: string, apiKey: string, model: string,
  adapter: StorageAdapter
): Promise<PromotionResult> {
  const memories = await adapter.getEpisodicMemoriesInWindow({
    profileId, windowStart, windowEnd, minLength: 20, limit: 2000,
  })
  if (memories.length === 0) return { promoted: 0, reinforced: 0, batches: 0 }
...
interface LLMEdge { source: string; target: string; relationship: string; confidence: number }

export interface GraphResult { edges_created: number }
...
export async function runGraphPass(
  ...
  if (nodes.length < 3) return { edges_created: 0 }
  ...
  for (const edge of edges) {
    ...
    if (!edge.relationship || typeof edge.confidence !== 'number') continue
    try {
