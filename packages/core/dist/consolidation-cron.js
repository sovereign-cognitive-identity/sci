...
async function run() {
    ...
    for (const profile of profiles) {
        ...
        if (result.batches === 0)
            console.log('no new memories in window');
        else
            console.log(`${result.promoted} promoted, ${result.reinforced} reinforced across ${result.batches} batches`);
    ...
    const digestResult = await runDigestPass(windowStart, windowEnd, profiles[0].id, { episodic_new: episodicNew, semantic_promoted: totalPromoted,
        semantic_reinforced: totalReinforced, nodes_decayed: totalDecayed }, OPENROUTER_KEY, MODEL, adapter);
    console.log(`  Stored: ${digestResult.memory_id ?? 'failed'}`);
    if (digestResult.exported_to)
        console.log(`  Exported: ${digestResult.exported_to}`);
...
//# sourceMappingURL=consolidation-cron.js.map