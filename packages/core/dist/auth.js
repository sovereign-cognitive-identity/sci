...
export async function validateToken(token) {
    ...
    writer.query(`UPDATE agent_tokens SET last_used_at = NOW() WHERE token_hash = $1`, [hash]).catch(() => { });
    return {
...
export async function registerAgent(name, tier, profileName) {
    // Resolve profile if given
    let profileId = null;
    let resolvedProfileName = null;
    if (profileName) {
        const { rows } = await reader.query('SELECT id, name FROM profiles WHERE name = $1', [profileName]);
        if (!rows[0])
    ...
    // Upsert agent
    const { rows: agentRows } = await writer.query(`INSERT INTO agents (name, tier, profile_id)
     VALUES ($1, $2, $3)
     ON CONFLICT (name) DO UPDATE SET tier = EXCLUDED.tier, profile_id = EXCLUDED.profile_id
     RETURNING id`, [name, tier, profileId]);
    ...
    await writer.query('DELETE FROM agent_tokens WHERE agent_id = $1', [agentId]);
    await writer.query('INSERT INTO agent_tokens (agent_id, token_hash, token_hint) VALUES ($1, $2, $3)', [agentId, tokenHash, tokenHint]);
    return { agentId, token, tokenHint, tier, profileId, profileName: resolvedProfileName };
}
export async function createConnectCode(agentName, tier, profileName) {
    let profileId = null;
    let resolvedProfileName = null;
    if (profileName) {
        const { rows } = await reader.query('SELECT id, name FROM profiles WHERE name = $1', [profileName]);
    ...
    const { rows } = await writer.query(`INSERT INTO connect_requests (code, agent_name, tier, profile_id)
     VALUES ($1, $2, $3, $4)
     RETURNING expires_at`, [code, agentName, tier, profileId]);
    return {
        code,
        agentName,
        tier,
...
export async function redeemConnectCode(code) {
    const { rows } = await reader.query('SELECT agent_name, tier, profile_id, expires_at, used_at FROM connect_requests WHERE code = $1', [code]);
    const req = rows[0];
    if (!req)
    ...
    // Mark as used
    await writer.query('UPDATE connect_requests SET used_at = NOW() WHERE code = $1', [code]);
    // Get profile name if scoped
    let profileName = null;
    if (req.profile_id) {
        const { rows: pRows } = await reader.query('SELECT name FROM profiles WHERE id = $1', [req.profile_id]);
...
export function canAccess(context, action) {
